...
getgenv().minGen = 1000000 -- Ignore anything below 1M
    ...
    while not getgenv().BrainrotFinderStop do
        ...
        if #all > 0 then
            ...
            if isNewServer or (not isSame) then
                -- CHECK 1: Minimum Value Filter
                if best.value >= (getgenv().minGen or 1000000) then
                    ...
                    -- Optional: Print locally but don't spam webhook
                    -- print("🗑️ Trash found:", best.name, best.value)
                end
            end
        end
